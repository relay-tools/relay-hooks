...
import { IEnvironment, GraphQLTaggedNode, Observer, Variables, getFragmentOwner } from 'relay-runtime';
...
const useOssFragment = function(fragmentDef, fragmentRef: any): FragmentResult {
    ...
    function newResolver() {
        const res = createFragmentSpecResolver({ environment }, 'useFragment', { frag: fragments }, propsFragments);
        res.setCallback(() => {
    ...
    const loadMore = useCallback(
        (connectionConfig: ConnectionConfig, pageSize: number, observerOrCallback: ObserverOrCallback, options: RefetchOptions) =>
    ...
    const refetchConnection = useCallback(
        (connectionConfig: ConnectionConfig, totalCount: number, callback: ObserverOrCallback, refetchVariables: Variables) =>
    ...
    return [
        data && data.frag ? (Array.isArray(data.frag) ? data.frag : { ...data.frag }) : null,
        {
            refetch,
            loadMore,
            hasMore,

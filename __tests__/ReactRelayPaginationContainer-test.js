...
const ReactRelayPaginationContainer = {
  createContainer: (Component, spec, connectionConfigs) => props => {
    const { user, ...others } = props;
    const environment = useRelayEnvironment();
    const [data, { hasMore: hasMoreHooks, isLoading, loadMore: loadMoreHooks, refetchConnection: refetchConnectionHooks}] = usePagination(spec, user);
    const loadMore = (...others) => {
      return loadMoreHooks(
        connectionConfigs,
        ...others
      );
    };
    const refetchConnection = (...others) => {
        return refetchConnectionHooks(
          connectionConfigs,
          ...others
        );
      };
      const hasMore = () => {
        return hasMoreHooks(connectionConfigs);
    };
    return (
      <Component user={data} {...others} relay={{ environment, refetchConnection, hasMore, isLoading, loadMore }} />
    );
  }
};
...
describe('ReactRelayPaginationContainer', () => {
  ...
  it('does not update for same props/data', () => {
    ...
    ); //changed expect(render).toBeCalled();
    expect(environment.lookup).not.toBeCalled();
    expect(environment.subscribe).not.toBeCalled();
  });
...
*/
  ...
  describe('loadMore()', () => {
    ...
    it('calls `getVariables` with props, count/cursor, and the previous variables', () => {
      loadMore(1, jest.fn());
      // changed not exist anymore user
